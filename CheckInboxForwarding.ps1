...
foreach ($u in $users) {
    $inboxRule = Get-InboxRule -Mailbox $u.UserPrincipalName | select Name, Enabled, RuleIdentity, ForwardTo, ForwardAsAttachmentTo
    ...
    foreach ($i in $inboxRule) {
        ...
        else
        {
            $property = [ordered]@{
                ...
                Enabled = if (($i.Enabled) -eq "True"){Write-Output "True"} else{Write-Output "False"}
